...
from pifsm.decorators import func_logger, func_timer
from sqlalchemy import or_, event
from sqlalchemy.orm import Session
from itertools import chain
import time
from flask_sqlalchemy.track_modications import before_models_committed
...
POKEDEX_HTML_PATH = 'pifsm/views/main/templates/pokemon_list.html'
...
def create_newdex_dict(base_dex_path=None, removed_dex_path=None):
    with (open(base_dex_path, newline='') as dexfile, open(removed_dex_path, newline='') as rdexfile):
        new_dex = chain(csv.DictReader(rdexfile), csv.DictReader(dexfile))
    return new_dex

def check_for_duplicates(new_dex):
...
def update_pokedex(initial_upload):
    ...
    if not initial_upload:
        print_changes(
            dex_additions=dex_additions,
            dex_deletions=dex_deletions,
...
def receive_before_flush(session, flush_context, instances):
    for obj in session.dirty:
        state = session.object_session(obj).inspect(obj)
        changes = {}
        for attr in state.attrs:
            hist = attr.history
            if hist.has_changes():
                changes[attr.key] = hist.added
        # Now changes contains the new values of the modified attributes
